...
import { createObjectFromError, ErrorType, InfraSightError, INFRA_SIGHT_API_URL } from '@infra-sight/sdk'
...
export const createScraperHandler = (
  ...
  try {
    ...
    const key = 'v2/cdn' + path + md5hex
    const location = INFRA_SIGHT_API_URL + key
...
export const createListHandler = (
  ...
  try {
    ...
    const response = await s3.send(new ListObjectsV2Command({
      ...
      Delimiter: '/',
      Prefix: 'v2/cdn' + path,
      ContinuationToken: event.queryStringParameters?.token
    }))
